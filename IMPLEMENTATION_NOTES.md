...
## 5차 (2026-09-24): 결과 영상을 방문자 기기에 저장

- R2 없는 배포에서 결과 영상은 fal 이 호스팅하는 주소뿐이라 영구 보관이 아니다. 완성되면 브라우저가 바로 받아 **IndexedDB** 에 넣고(`src/lib/client/local-videos.ts`), 목록·상세는 그 사본을 재생한다(`useLocalVideo`). 처음 저장할 때 `navigator.storage.persist()` 로 보존을 요청한다.
- **서버를 거치지 않는다.** fal 미디어 주소(`v3*.fal.media`)가 `access-control-allow-origin: *` 를 주는 것을 확인했다. 로컬 모드 결과는 같은 오리진 `/api/files/…`.
- **확인이 끝나기 전에는 원격 주소를 주지 않는다.** 처음에 훅의 초기 상태를 원격 주소로 두었더니, 기기 사본을 찾기 전에 `<video>` 가 원격 파일을 받기 시작했다(재방문마다 206 요청 3건). 초기 `src` 를 비우고 호출부의 원격 폴백을 없애 재방문 원격 요청 0건을 확인했다.
- 목록과 상세가 같은 영상을 동시에 저장하지 않도록 진행 중인 저장을 작업 id 로 공유한다.
- **검토 반영(판정자+반박자)**: 목록을 열면 사본 없는 완료 영상을 최대 100개까지 동시에 받던 것을 **동시 2개 대기열**로 제한하고 상세 화면 요청을 앞에 세운다. 저장이 같은 화면에서 끝나면 재생 중인 원격 영상은 그대로 두고 다운로드만 기기 사본으로 바꾼다(바꾸면 처음부터 다시 재생된다). IndexedDB 연결이 끊기면 다시 열고, open·조회·받기에 시간 제한을 둔다. 링크 만료·용량 부족 같은 영구 실패는 세션 동안 기억해 목록에서 다시 받지 않고, 오류는 한국어로 분류한다. `persist()` 는 다운로드 클릭에서만 요청한다(Firefox 권한 팝업). "이 기기에서 지우기"는 다시 저장하기 전까지 자동 저장을 끈다.
- 자동화 브라우저에서 결과 화면이 "생성 중"에 멈춰 보인 것은 탭이 `visibilityState: hidden` 이라 SWR 폴링이 쉰 탓이었다(버그 아님, 탭이 보이면 재개).
